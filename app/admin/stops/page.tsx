          ...
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y divide-brand-cream/10">
              {rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr key={row.id} className="hover:bg-brand-cream/[0.03]">
                      ...
                      <Link href={`/admin/stops/${row.id}`} className="block group">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <div className="text-brand-cream font-medium group-hover:text-brand-orange transition-colors truncate">{row.name}</div>
                            {row.description && (
                              <div className="text-xs text-brand-cream/55 mt-0.5 line-clamp-2">{row.description}</div>
                            )}
                          </div>
                        </div>
                      </Link>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-brand-cream/80">
                      {row.category_name ?? <span className="text-brand-cream/40">—</span>}
                    </td>
                    <td className="px-4 py-3 text-brand-cream/75 whitespace-nowrap">{row.area ?? "—"}</td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="flex gap-1">
                        {row.requires_booking && <span className="px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider bg-brand-orange/20 text-brand-orange">€</span>}
                        {row.is_seasonal && <span className="px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider bg-brand-cream/10 text-brand-cream/70" title="Seasonal">SE</span>}
