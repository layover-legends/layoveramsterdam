...
export default function TranslationsEditor({ entityType, entityId, fields, existing }: Props) {
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      ...
      <div className="rounded-xl border border-brand-cream/10 bg-brand-cream/[0.03] p-5 space-y-4">
          ...
          <div key={f.key}>
            ...
            {f.multiline ? (
              <textarea
                ...
                placeholder={`${f.label} in ${LOCALES.find((l) => l.code === activeLocale)?.label ?? activeLocale}…`}
              />
            ) : (
              <input
                type="text"
        ...
        >
          {isPending ? "Saving…" : `Save ${LOCALES.find((l) => l.code === activeLocale)?.label ?? activeLocale}`}
        </button>
      </div>
    </div>
  );
