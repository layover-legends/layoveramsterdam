            ...
            {rows.map((row) => (
              <tr key={row.id} className="hover:bg-warm-cream/[0.02] transition-colors">
                ...
                <td className="px-4 py-3 hidden sm:table-cell">
                  <span className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded border ${TYPE_BADGE[row.service_type] ?? ""}`}>
                    {row.service_type}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <AvailabilityToggle serviceId={row.id} current={row.availability_status} />
                </td>
